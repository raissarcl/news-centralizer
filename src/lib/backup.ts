...
export async function exportBackupJson(): Promise<BackupResult> {
  try {
    ...
    return { ok: true, message: t.exportBackup };
  } catch {
    return { ok: false, message: t.importFailed };
  }
}

export async function importBackupJson(): Promise<BackupResult> {
  const result = await DocumentPicker.getDocumentAsync({
    type: 'application/json',
    copyToCacheDirectory: true,
  });
  if (result.canceled || !result.assets?.[0]?.uri) {
    return { ok: false, message: t.importFailed, canceled: true };
  }
...
export async function exportOpml(): Promise<BackupResult> {
  try {
    ...
    return { ok: true, message: t.exportOpml };
  } catch {
    return { ok: false, message: t.importFailed };
  }
}
...
export async function importOpml(
  ...
  try {
    ...
    return {
      ok: true,
      message: importResultMessage(added, skipped, t.opmlImported),
    };
  } catch {
    return { ok: false, message: t.importFailed };
  }
