...
export function FolderDetailScreen() {
      ...
      headerRight: () => (
        <View style={styles.headerActions}>
          {unread > 0 ? (
            ...
            >
              <Text style={{ color: tokens.primary, fontSize: 14 }}>{t.markAllRead}</Text>
            </Pressable>
          ) : null}
          <Pressable onPress={openFolderMenu} hitSlop={8} style={{ paddingHorizontal: 8 }}>
            <Ionicons name="ellipsis-horizontal" size={20} color={tokens.text} />
          </Pressable>
        </View>
      ),
    });
  }, [folder, navigation, markAllReadInFolder, unread, tokens.primary, tokens.text, openFolderMenu]);
  ...
  const listHeader = (
    <View style={[styles.banner, { backgroundColor: tokens.surface, borderColor: tokens.border }]}>
      ...
      <Pressable
        onPress={() => router.push(`/folder/${folder.id}/feeds`)}
        style={[styles.manageBtn, { backgroundColor: tokens.surfaceAlt, borderColor: tokens.border }]}
      >
