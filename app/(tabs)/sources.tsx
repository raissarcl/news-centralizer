...
import { FeedHealthBanner } from '@/features/feeds/components/FeedHealthBanner';
...
import { isRssHubUrl } from '@/lib/rsshub';
import {
  validateFeedUrl,
  feedUrlErrorMessage,
...
export default function SourcesScreen() {
  ...
  const rssHubAcknowledged = useSettingsStore(
    (s) => s.settings.rssHubAcknowledged,
  );
    ...
    return (
      ...
      >
        ...
        <View style={styles.main}>
          ...
          <Text
            style={[styles.sub, { color: tokens.textMuted }]}
            numberOfLines={1}
          >
          ...
          new Date(item.refreshPausedUntil).getTime() > Date.now() ? (
            <Text style={[styles.error, { color: tokens.textFaint }]}>
              {t.feedPaused}
            </Text>
          ) : null}
          {item.lastError ? (
        ...
        {isFeedPausedNow(item) ? (
          <Pressable onPress={() => void resumeFeed(item.id)} hitSlop={8}>
            <Text style={{ color: tokens.primary, fontSize: 12 }}>
              {t.resumeFeed}
            </Text>
          </Pressable>
        ) : null}
  ...
  const listHeader = (
    <View style={styles.header}>
      ...
      <FeedHealthBanner
        ...
        onResumeAll={() => void resumeAllPausedFeeds()}
      />
      ...
      >
        <Pressable
          ...
          style={[
            styles.folderChip,
            {
              backgroundColor: !folderFilterId
                ? tokens.primary
                : tokens.surfaceAlt,
              borderColor: tokens.border,
            },
